...
class IrActionsActWindow(models.Model):
    ...
    def ecrud_almost_everyone(self, do_a_filter=False, system_option=True):
        ...
        return (
            window_actions.filtered(self._ecrud_check_name(system_option))
            if do_a_filter
            else window_actions
        )
    ...
    def ecrud_get_from_app_data(
        ...
        window_actions = self.browse(
            self.env["ir.model.data"].ecrud_get_data(app).mapped("res_id")
        ).filtered(current_target_lambda)
        return (
            window_actions.filtered(self._ecrud_check_name(system_option))
            if do_a_filter
            else window_actions
...
class EnhancedCrudActWindowLink(models.TransientModel):
    ...
    def link_window_actions(self):
        ...
        if opcion not in ("all", "system", "nosystem"):
            window_actions = ir_act_window.ecrud_get_from_app_data(
                opcion, do_a_filter=True, system_option=False
            )

        else:
            window_actions = ir_act_window.ecrud_almost_everyone(
                do_a_filter=opcion != "all", system_option=opcion == "system"
            )
